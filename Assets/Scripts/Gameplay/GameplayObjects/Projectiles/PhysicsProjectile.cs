    ...
    public class PhysicsProjectile : NetworkBehaviour
    {
        ...
        public override void OnNetworkSpawn()
        {
            if (IsServer)
            {
                ...
                if (DuelSessionState.Instance != null && DuelSessionState.Instance.IsPvp)
                {
                    m_CollisionMask |= LayerMask.GetMask(new[] { "PCs" });
                }
        ...
        void DetectCollisions()
        {
            ...
            for (int i = 0; i < numCollisions; i++)
            {
                ...
                bool isCharacterLayer = m_CollisionCache[i].gameObject.layer == m_NpcLayer ||
                                        (DuelSessionState.Instance != null && DuelSessionState.Instance.IsPvp &&
                                         m_CollisionCache[i].gameObject.layer == m_PcLayer);

                if (isCharacterLayer && !m_HitTargets.Contains(m_CollisionCache[i].gameObject))
                {
                    ...
                    if (targetNetObj)
                    {
                        ...
                        var targetCharacter = targetNetObj.GetComponent<ServerCharacter>();

                        if (targetCharacter != null && spawnerObj != null &&
                            !ActionUtils.IsValidActionTarget(spawnerObj, targetCharacter, false))
                        {
                            continue;
                        }
