...
namespace Unity.BossRoom.Gameplay.GameplayObjects.Character
{
    ...
    public class ServerCharacter : NetworkBehaviour, ITargetable
    {
        ...
        int m_MaxHitPointsOverride;

        public int MaxHitPoints => m_MaxHitPointsOverride > 0 ? m_MaxHitPointsOverride : CharacterClass.BaseHP.Value;

        public float DamageDealtMultiplier { get; set; } = 1f;
        ...
        public void SetMaxHitPointsMultiplier(float multiplier, bool healToFull)
        {
            multiplier = Mathf.Max(1f, multiplier);
            m_MaxHitPointsOverride = Mathf.RoundToInt(CharacterClass.BaseHP.Value * multiplier);
        ...
        void ReceiveHP(ServerCharacter inflicter, int HP)
        {
            ...
            else
            {
                ...
                if (inflicter != null && inflicter.DamageDealtMultiplier > 0f &&
                    !Mathf.Approximately(inflicter.DamageDealtMultiplier, 1f))
                {
                    HP = -Mathf.Max(1, Mathf.RoundToInt(Mathf.Abs(HP) * inflicter.DamageDealtMultiplier));
                }
