...
namespace Unity.BossRoom.Gameplay.GameplayObjects
{
    public class GameDataSource : MonoBehaviour
    {
        ...
        public IReadOnlyList<Action> GetCampaignRewardCandidates()
        {
            var candidates = new List<Action>();
            foreach (var action in m_AllActions)
            {
                if (IsCampaignRewardCandidate(action))
                {
        ...
        static bool IsCampaignRewardCandidate(Action action)
        {
            if (action == null || action.Config == null || action.Config.Icon == null ||
                string.IsNullOrWhiteSpace(action.Config.DisplayedName))
            {
                return false;
            }

            switch (action.Config.Logic)
            {
                case ActionLogic.Chase:
