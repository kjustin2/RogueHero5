...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    public class AOEAction : Action
    {
        ...
        private void PerformAoE(ServerCharacter parent)
        {
            ...
            if (DuelSessionState.Instance != null && DuelSessionState.Instance.IsPvp && !parent.IsNpc)
            {
                layerMask |= LayerMask.GetMask("PCs");
            }

            var colliders = Physics.OverlapSphere(m_Data.Position, Config.Radius, layerMask);
            for (var i = 0; i < colliders.Length; i++)
            {
                var enemy = colliders[i].GetComponent<IDamageable>();
                var targetCharacter = colliders[i].GetComponentInParent<ServerCharacter>();
                if (enemy != null &&
                    (targetCharacter == null || ActionUtils.IsValidActionTarget(parent, targetCharacter, Config.IsFriendly)))
                {
