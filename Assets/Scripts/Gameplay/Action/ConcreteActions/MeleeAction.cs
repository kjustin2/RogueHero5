...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    public partial class MeleeAction : Action
    {
        ...
        {
            return GetIdealMeleeFoe(parent, Config.IsFriendly, parent.physicsWrapper.DamageCollider, Config.Range, Config.Radius, foeHint);
        }
        ...
        public static IDamageable GetIdealMeleeFoe(ServerCharacter attacker, bool isFriendlyAction, Collider ourCollider, float meleeRange, float meleeRadius, ulong preferredTargetNetworkId)
        {
            RaycastHit[] results;
            int numResults = ActionUtils.DetectNearbyActionTargets(attacker, isFriendlyAction, ourCollider, meleeRange, meleeRadius, out results);
            ...
            for (int i = 0; i < numResults; i++)
            {
                ...
                if (TryGetServerCharacter(damageable.NetworkObjectId, out var targetCharacter) &&
                    !ActionUtils.IsValidActionTarget(attacker, targetCharacter, isFriendlyAction))
                {
                    continue;
                }
        ...
        static bool TryGetServerCharacter(ulong networkObjectId, out ServerCharacter serverCharacter)
        {
            serverCharacter = null;
            return Unity.Netcode.NetworkManager.Singleton.SpawnManager.SpawnedObjects.TryGetValue(networkObjectId, out var networkObject) &&
                   networkObject.TryGetComponent(out serverCharacter);
        }
    }
}
