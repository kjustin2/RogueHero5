...
namespace Unity.BossRoom.Gameplay.Actions
{
    public static class ActionUtils
    {
        ...
        public static int DetectNearbyActionTargets(ServerCharacter attacker, bool isFriendlyAction, Collider attackerCollider, float range, float radius, out RaycastHit[] results)
        {
            GetActionTargetLayers(attacker, isFriendlyAction, out bool wantPcs, out bool wantNpcs);
            return 0.0f < radius
                ? DetectNearbyEntitiesUseSphere(wantPcs, wantNpcs, attackerCollider, range, radius, out results)
                : DetectNearbyEntities(wantPcs, wantNpcs, attackerCollider, range, out results);
        }

        public static bool IsValidActionTarget(ServerCharacter attacker, ServerCharacter target, bool isFriendlyAction)
        {
            ...
            if (attacker == target && !isFriendlyAction)
            {
                return false;
            }

            if (IsPvpPlayerOpponent(attacker, target, isFriendlyAction))
            {
                return true;
            }

            return target.IsNpc != (isFriendlyAction ^ attacker.IsNpc);
        }

        public static bool IsPvpPlayerOpponent(ServerCharacter attacker, ServerCharacter target, bool isFriendlyAction)
        {
            return DuelSessionState.Instance != null &&
                   DuelSessionState.Instance.IsPvp &&
                   !isFriendlyAction &&
                   attacker != null &&
                   target != null &&
                   attacker != target &&
        ...
        static void GetActionTargetLayers(ServerCharacter attacker, bool isFriendlyAction, out bool wantPcs, out bool wantNpcs)
        {
            bool targetSelector = isFriendlyAction ^ attacker.IsNpc;
            wantPcs = false != targetSelector;
            wantNpcs = true != targetSelector;

            if (DuelSessionState.Instance != null &&
                DuelSessionState.Instance.IsPvp &&
                !isFriendlyAction &&
                !attacker.IsNpc)
            {
